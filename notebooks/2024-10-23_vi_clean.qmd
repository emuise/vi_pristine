...
indiv_plot_joined2 <- indiv_plot_joined %>%
  left_join(alpha_join)
...
indiv_tukeys <- indiv_plot_joined2 %>% 
...
panel12 <- free(vp[["elev_p95"]]) + free(vp[["percentage_first_returns_above_2m"]]) + free(vp[["clim_MAP"]]) + free(vp[["clim_MAT"]]) +
  free(vp[["elev_cv"]]) + free(vp[["total_biomass"]]) + free(vp[["clim_MCMT"]]) + free(vp[["clim_MWMT"]]) +
  free(vp[["CumDHI"]]) + free(vp[["VarDHI"]]) + free(vp[["slope"]]) + free(vp[["DEM"]]) + hf_map_panel + inset + plot_layout(design = layout)

ggsave(here::here(figure_loc, "data_panels.png"), panel12, dpi = 600, height = 20, width = 20)
ggsave(here::here(figure_loc, "data_panels.pdf"), panel12, dpi = 600, height = 20, width = 20)
...
twoby <- vp[["elev_p95"]] + vp[["CumDHI"]] + hf_map_panel + vp[["clim_MAP"]] + vp[["slope"]] + plot_layout(nrow = 2) + plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(hjust = 1, vjust = 0))
ggsave(here::here(figure_loc, "data_figure.png"), twoby, height = 12, width = 12)
```
...
# join table for the short names used for saving mahal files
cleaner_join <- tibble(variable = all_vars, filter = all_vars_names) %>% left_join(keys$continuous)

subsets <- vect("E:/Sync/Masters/04_vi_pristine/strathcona/data/shapefiles/subset_SA.shp")
...
mahal_files <- tibble(short_name = all_vars_names,
       group = c(rep("str", 4), rep("dhi", 3))) %>%
  mutate(file = here::here(mahal_ras_loc, glue("{short_name}-0.9_{group}.dat"))) %>%
  filter(file.exists(file)) %>%
  pull(file)

mahal_rasters <- mahal_files %>%
  rast() %>%
...
panels <- subsets %>% 
  ...
  map(\(x) {
    title = x %>%
      pull(label)
    subtitle = NULL
    mahal_subtitles <- map_chr(mahal_rasters, names) %>%
      as_tibble() %>%
      separate(value, into = c("filter", "filter1", "filter2", "class")) %>%
      left_join(cleaner_join) %>%
      select(var_long, class) %>%
      mutate(
        subtitle = glue(
          "{ifelse(class == 'str', 'Structural', 'Functional')} Similarity to Top 10% {var_long}"
        )
      ) %>%
      pull(subtitle) %>%
      str_wrap(width = 20)
    ...
    hf_cropped <- ggplot() +
      ...
      geom_spatvector(data = scale_box, col = "white", fill = "white") +
      geom_spatvector_text(data = subset_labels %>%
                             filter(label == title), aes(label = label), col = "red", size = 6, fontface = "bold", hjust = 0, vjust = 1) +
      map_theme +
      theme(
    ...
    mahal_cropped <- map2(mahal_rasters, mahal_subtitles, \(r, st) {
    ...
    mahal_cropped[[7]] <- hf_cropped
    
    return(mahal_cropped)
...
bacon_wide <- ((hf_multi_inset_base + theme(legend.position = "bottom")) +
            panels[[7]] + 
            (panels[[1]] + theme(legend.position = "none")) + 
            (panels[[2]] + theme(legend.position = "none")) + 
            (panels[[3]] + theme(legend.position = "none")) + 
            (panels[[4]] + theme(legend.position = "none")) + 
            (panels[[5]] + theme(legend.position = "none")) + 
            (panels[[6]] + theme(legend.position = "none")) +
  (panels[[14]] + theme(legend.position = "none")) + 
    (panels[[8]] + theme(legend.position = "none")) + 
    (panels[[9]] + theme(legend.position = "none")) +
    (panels[[10]] + theme(legend.position = "none")) + 
    (panels[[11]] + theme(legend.position = "none")) + 
    (panels[[12]] + theme(legend.position = "none")) + 
    (panels[[13]] + theme(legend.position = "none")) +
  (panels[[21]] + theme(legend.position = "none")) + 
    (panels[[15]] + theme(legend.position = "none")) + 
    (panels[[16]] + theme(legend.position = "none")) + 
    (panels[[17]] + theme(legend.position = "none")) + 
    (panels[[18]] + theme(legend.position = "none")) + 
    (panels[[19]] + theme(legend.position = "none")) + 
    panels[[20]] +
  plot_layout(guides = "collect") & theme(legend.position = "right",
                                          plot.margin = margin(0, 0, 0, 0))) +
  plot_layout(design = sigma_layout_wide)
  
ggsave(here::here(figure_loc, "multi_panel_inset_wide.png"), bacon_wide, height = 7, width = 14)
  ...
  map(\(x) {
    ...
    hf_cropped <- ggplot() +
      ...
      scale_fill_manual(guide = guide_legend(title = "Human Footprint", nrow = 2),
                        ...
                        na.value = "transparent",
                        na.translate = F) +
      # tx says north arrow not required when is where is expected
    ...
    if(title == "A") {
      hf_cropped <- hf_cropped +
        facet_grid("Human Footprint"~., switch = "y") +
    ...
    mahal_cropped <- map2(mahal_rasters, mahal_subtitles, \(r, st) {
      ...
      if (title == "A") {
        mahal_out <- mahal_out +
          facet_wrap(enquo(st), strip.position = "left")
      }
    })
...
bacon_tall <- hf_multi_inset_base + #A
            panels_tall[[7]] + panels_tall[[14]] + panels_tall[[21]] + #BCD
            panels_tall[[1]] + panels_tall[[8]] + panels_tall[[15]] + #EFG
            panels_tall[[2]] + panels_tall[[9]] + panels_tall[[16]] + #HIJ
            panels_tall[[3]] + panels_tall[[10]] + panels_tall[[17]] + #EFG
            panels_tall[[4]] + panels_tall[[11]] + panels_tall[[18]] + #HIJ
            panels_tall[[5]] + panels_tall[[12]] + panels_tall[[19]] + #EFG
            panels_tall[[6]] + panels_tall[[13]] + panels_tall[[20]] + #HIJ
  plot_layout(design = sigma_layout_long, guides = "collect") & theme(legend.position = "right")

ggsave(here::here(figure_loc, "multi_panel_inset_long.png"), bacon_tall, height = 11, width = 7)
```
...
ggplot() +
      ...
      theme(
        ...
        plot.background = element_blank(),
        panel.border = element_blank()) +
      labs(title = subtitle, subtitle = title) +
      ...
      scico::scale_fill_scico(
        ...
        labels = c(
          "No Pressure (0)",
          "Low Pressure (0-4)",
          "Medium Pressure (4-8)", "High Pressure (8+)"
        )
      )
...
ggplot() +
      ...
      theme(
        ...
        panel.border = element_blank()) +
      labs(title = subtitle, subtitle = title) +
      scale_fill_manual(labels = hf_tib_sub$labels,
                        values = hf_tib_sub$values,
                        na.value = "transparent",
                        na.translate = F) +
      ggspatial::annotation_north_arrow(
